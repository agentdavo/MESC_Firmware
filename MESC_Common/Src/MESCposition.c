...
void RunPosControl(MESC_motor* _motor)
{
  ...
  //Clamp the integral
  if (_motor->pos.int_error > input_vars.max_request_Idq.q)
    {
      _motor->pos.int_error = input_vars.max_request_Idq.q;
    }
  if (_motor->pos.int_error < input_vars.min_request_Idq.q)
    {
      _motor->pos.int_error = input_vars.min_request_Idq.q;
    }
